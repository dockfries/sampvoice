...
---------------------------------
Этот форк добавляет ряд улучшений клиентской части поверх порта под open.mp:

* **Внешние ресурсы интерфейса** — клиент загружает шрифты, иконки, шейдер размытия
  и языковые пакеты из папок `sampvoice/resources/` и `sampvoice/languages/`,
  расположенных рядом с `.asi`, вместо встраивания. Ни один ресурс интерфейса
  не компилируется в бинарный файл.
  * Пользователь может заменить `sampvoice/resources/font.ttf` (или `font.otf`),
    чтобы сменить шрифт интерфейса. В атлас глифов включены: латиница, кириллица,
    греческий, иврит, арабский (только глифы, без RTL), полный CJK, японская
    кана, корейский и тайский.
* **Языковые пакеты во время выполнения** — строки меню загружаются из
  `sampvoice/languages/<имя>.json` (UTF-8, пары ключ/значение) с запасным
  вариантом на английском. В вкладке «Общие» есть селектор языка, который
  переключает пакеты на лету и сохраняет выбор. В комплекте: English, Русский,
  Srpski, Bahasa Indonesia, Português (Brasil), 简体中文.
* **Многобайтовый ввод и отображение** — `WM_CHAR`/`WM_IME_CHAR` передаются
  напрямую в ImGui (работает китайский ввод через IME); ники игроков
  конвертируются из системной ANSI-кодовой страницы в UTF-8 для отображения;
  фильтр чёрного списка нечувствителен к регистру и конвертирует ввод в
  системную кодовую страницу перед сравнением.
* **Исправление доступности микрофона** — устройства записи перечисляются
  локально при загрузке игры, а не только во время рукопожатия с сервером,
  поэтому меню больше не сообщает «нет доступных микрофонов», когда на сервере
  нет плагина.
* **Оптимизация задержки PTT** — убрано логирование каждого нажатия клавиши,
  канал записи BASS остаётся активным при включённом микрофоне, а накопленные
  данные сбрасываются вместо паузы/воспроизведения при каждом нажатии.
* **Совместимость с omp-cef** — рефакторинг жизненного цикла рендера: окно
  обрабатывается через `SetWindowSubclass` вместо `SetWindowLong`, точка вызова
  `Direct3DCreate9` перехватывается с проверкой байтов, а обёртки D3D9 используют
  внутренний COM-подсчёт ссылок.
* **Dear ImGui обновлён до v1.92.9b** (с 1.68 WIP) и адаптирован к новому API
  (`BeginChild(ImGuiChildFlags_FrameStyle)`, `ImTextureRef`, самоуправляемый
  атлас, данные шрифта хранятся в течение жизни атласа).
