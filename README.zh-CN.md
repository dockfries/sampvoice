...
* **外部 UI 资源** —— 客户端从 `.asi` 同级的 `sampvoice/resources/` 和 `sampvoice/languages/` 文件夹加载字体、图标、模糊着色器和语言包，不再编译进二进制。没有 UI 资源被打进 `.asi`。
  * 用户可替换 `sampvoice/resources/font.ttf`（或 `font.otf`）来更换 UI 字体。烘焙进图集（atlas）的字形覆盖：拉丁、西里尔、希腊、希伯来、阿拉伯（仅有字形，无 RTL 排版）、全量 CJK、日文假名、韩文谚文和泰文。
* **运行时语言包** —— 菜单文案从 `sampvoice/languages/<名称>.json`（UTF-8，键值对）加载，并带英文兜底。在"常规"页有语言选择下拉框，可运行时切换并持久化。内置语言：English、Русский、Srpski、Bahasa Indonesia、Português (Brasil)、简体中文。
* **多字节输入与显示** —— `WM_CHAR`/`WM_IME_CHAR` 直接送入 ImGui（支持输入法合成的中文输入）；玩家昵称从系统 ANSI 代码页转换为 UTF-8 显示；黑名单筛选大小写不敏感，匹配前会把输入归一化到系统代码页。
